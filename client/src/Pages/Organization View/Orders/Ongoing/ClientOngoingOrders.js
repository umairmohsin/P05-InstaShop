...
const ClientOngoingOrders = ()=>{
    ...
    const myfunc = ()=>{
        pendinglist.map((val,key)=>{
            if(val.clientEmail === email && val.status ==="Ongoing" && val.acceptedByClient === false){
                newpendlist[key] = val
            }
        })
            ...
            <div className='ipa'>
                <h2>Here are your ongoing orders</h2>
                {
                    newpendlist.map((val,key)=>{
                        return(
                            <div className='clientOngoingOrderCard'>
                                <div>
...
export default ClientOngoingOrders;